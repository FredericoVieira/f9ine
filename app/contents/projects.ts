import studio from "@/public/projects/studio.png";
import mobileBuilder from "@/public/projects/mobile-builder.png";
import nordy from "@/public/projects/nordy.png";
import hp from "@/public/projects/hp.png";
import lilly from "@/public/projects/lilly.png";
import cropwiseProtector from "@/public/projects/cropwise-protector.png";
import izzi from "@/public/projects/izzi.png";
import bdmg from "@/public/projects/bdmg.png";
import kim from "@/public/projects/kim.png";
import simplicidata from "@/public/projects/simplicidata.png";
import dataviva from "@/public/projects/dataviva.png";
import gravol from "@/public/projects/gravol.png";

const projects = [
  {
    image: studio,
    title: "Studio",
    description:
      "Web platform to manage and analyze user engagement and product adoption.",
    url: "https://www.appcues.com/",
    displayUrl: "appcues.com",
  },
  {
    image: mobileBuilder,
    title: "Mobile Builder",
    description:
      "Web platform to create in-app mobile flows through a no-code builder experience.",
    url: "https://www.appcues.com/mobile",
    displayUrl: "appcues.com/mobile",
  },
  {
    image: nordy,
    title: "Nordy",
    description:
      "Web dictionary with words and meanings learned by me on a daily basis.",
    url: "https://nordy.vercel.app",
    displayUrl: "nordy.vercel.app",
    source: "https://github.com/FredericoVieira/nordy",
  },
  {
    image: hp,
    title: "HP Print Service",
    description:
      "Printer subscription manager, with history, paper usage, cartridge status, and toner levels.",
    url: "https://www.hp.com/us-en/solutions/business-solutions/print-at-your-service.html",
    displayUrl: "hp.com/print-service",
  },
  {
    image: lilly,
    title: "Lilly Tempo Website",
    description:
      "Personalized Diabetes Management Platform for patients and healthcare professionals.",
    url: "https://www.lillytempo.com",
    displayUrl: "lillytempo.com",
  },
  {
    image: cropwiseProtector,
    title: "Cropwise Protector",
    description:
      "Agricultural platform with georeferenced monitoring and pest control.",
    url: "https://www.cropwise.com/protector",
    displayUrl: "cropwise.com/protector",
  },
  {
    image: izzi,
    title: "IZZI Gaming",
    description:
      "Web platform of coaching, courses, and game replay analysis to improve e-players skills.",
    url: "https://izzi.gg",
    displayUrl: "izzi.gg",
  },
  {
    image: bdmg,
    title: "BDMG App",
    description:
      "Security contract management app with proof of life and documents validation.",
    url: "https://apps.apple.com/br/app/bdmg/id1507682398",
    displayUrl: "apps.apple.com/bdmg",
  },
  {
    image: kim,
    title: "KIM+ App",
    description:
      "Service management app with bills payment, phone data recharge and discounts offers.",
    url: "https://kimmais.app",
    displayUrl: "kimmais.app",
  },
  {
    image: simplicidata,
    title: "Simplicidata",
    description:
      "A.I. web platform with standard and customized data transformations per client.",
  },
  {
    image: dataviva,
    title: "Dataviva",
    description:
      "Collaborative visualization platform of Brazilian socioeconomic data, with data analysis and comparison.",
    url: "http://dataviva.info/pt",
    displayUrl: "dataviva.info",
    source: "https://github.com/DataViva",
  },
  {
    image: gravol,
    title: "Gravol",
    description:
      "Web platform to collect and organize sports teams performance data by storing match events and videos.",
  },
];

export default projects;
